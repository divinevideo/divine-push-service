...
- `coalesce:g:{type}:{owner}:{target}:{bucket}` - Like/repost bucket group (pending/immediate counts, first actor, routing fields, flush lease). TTL `coalesce_window_secs + coalesce_logical_expiry_grace_secs`
...
- `coalesce:throttle:{owner}` - Per-recipient token bucket for immediate like/repost pushes
