...
#[tokio::test]
async fn test_coordinate_claim_suppresses_edit_but_allows_new_recipient() {
    let Some(pool) = create_test_pool().await else {
        return;
    };

    let owner = Keys::generate();
    let original_recipient = Keys::generate();
    let added_recipient = Keys::generate();
    let coordinate = format!("34236:{}:video-id", owner.public_key().to_hex());
    let original_claim_key = format!(
        "{}:{}",
        coordinate,
        original_recipient.public_key().to_hex()
    );
    let added_claim_key = format!("{}:{}", coordinate, added_recipient.public_key().to_hex());

    assert!(
        redis_store::try_claim_dedup_key(&pool, &original_claim_key, 30)
            .await
            .unwrap(),
        "initial publish should notify the original recipient"
    );
    assert!(
        !redis_store::try_claim_dedup_key(&pool, &original_claim_key, 30)
            .await
            .unwrap(),
        "an edit should not notify the original recipient again"
    );
    assert!(
        redis_store::try_claim_dedup_key(&pool, &added_claim_key, 30)
            .await
            .unwrap(),
        "an edit should notify a newly added recipient"
    );
}
